...
echo_latest_version() {
  if [ "${EDGE-}" ]; then
    version="$(curl -fsSL https://api.github.com/repos/nxpkg/code-server/releases | awk 'match($0,/.*"html_url": "(.*\/releases\/tag\/.*)".*/)' | head -n 1 | awk -F '"' '{print $4}')"
  else
    # https://gist.github.com/lukechilds/a83e1d7127b78fef38c2914c4ececc3c#gistcomment-2758860
    version="$(curl -fsSLI -o /dev/null -w "%{url_effective}" https://github.com/nxpkg/code-server/releases/latest)"
  fi
  version="${version#https://github.com/nxpkg/code-server/releases/tag/}"
  version="${version#v}"
  echo "$version"
}
...
echo_coder_postinstall() {
  echoh
  echoh "Deploy code-server for your team with Coder: https://github.com/nxpkg/coder"
}
...
install_deb() {
  ...
  fetch "https://github.com/nxpkg/code-server/releases/download/v$VERSION/code-server_${VERSION}_$ARCH.deb" \
...
install_rpm() {
  ...
  fetch "https://github.com/nxpkg/code-server/releases/download/v$VERSION/code-server-$VERSION-$ARCH.rpm" \
...
install_standalone() {
  ...
  fetch "https://github.com/nxpkg/code-server/releases/download/v$VERSION/code-server-$VERSION-$OS-$ARCH.tar.gz" \
